...
: Node("pure_pursuit", node_options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
{
  ...
  // Vehicle Parameters from vehicle_info_util.hpp
  // const auto vehicle_info = vehicle_info_util::VehicleInfoUtil(*this).getVehicleInfo();
  param_.wheel_base = this->declare_parameter<double>("wheel_base",2.875);
  // RCLCPP_INFO(this->get_logger(),"The wheel base now is %f", param_.wheel_base);
  ...
  sub_trajectory_ = this->create_subscription<autoware_auto_planning_msgs::msg::Trajectory>(
    "pure_pursuit/input/reference_trajectory", 1, std::bind(&PurePursuitNode::onTrajectory, this, _1));
  sub_current_odometry_ = this->create_subscription<nav_msgs::msg::Odometry>(
    "pure_pursuit/input/current_odometry", 1, std::bind(&PurePursuitNode::onCurrentOdometry, this, _1));

  // Publishers
  pub_ctrl_cmd_ = this->create_publisher<autoware_auto_control_msgs::msg::AckermannLateralCommand>(
    "pure_pursuit/output/control_raw", 1);
  ...
  //  Wait for first current pose, base_link is the center point on the rear axel
...
bool PurePursuitNode::isDataReady()
{
  ...
  if (!current_pose_) {  //current_pose is ??
...
void PurePursuitNode::onTimer()
{
  ...
  if (target_curvature) {
    ...
    // RCLCPP_INFO(this->get_logger(), "Failed to solve pure_pursuit.Brake!");
    //publish a brake signal
    // publishCommand({0.0});
    autoware_auto_control_msgs::msg::AckermannLateralCommand cmd;
    cmd.stamp = get_clock()->now();
    cmd.steering_tire_angle = 0.0;
    cmd.steering_tire_rotation_rate = -1.0;
...
PurePursuitNode::calcTargetPoint() const
{
  const auto closest_idx_result = planning_utils::findClosestIdxWithDistAngThr(
    planning_utils::extractPoses(*trajectory_), current_pose_->pose, 5.5, M_PI_4);

  if (!closest_idx_result.first) {
    // RCLCPP_ERROR(get_logger(), "cannot find closest waypoint, brake!");
    return {};
  }
