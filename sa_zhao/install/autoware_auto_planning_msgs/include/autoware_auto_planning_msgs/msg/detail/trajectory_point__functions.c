  ...
  autoware_auto_planning_msgs__msg__TrajectoryPoint__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_auto_planning_msgs__msg__TrajectoryPoint * data =
      (autoware_auto_planning_msgs__msg__TrajectoryPoint *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_planning_msgs__msg__TrajectoryPoint__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_auto_planning_msgs__msg__TrajectoryPoint__fini(&data[i]);
        }
        free(data);
        return false;
      }
